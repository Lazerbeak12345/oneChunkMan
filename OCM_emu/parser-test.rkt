...
(require rackunit
         "tokenizer.rkt"
         "parser.rkt")
(test-equal? "comments work" (parse-to-datum (__apt/mt "(comment)\n")) '(ocm-asm #f))
(test-equal?
 "labels work"
 (parse-to-datum (__apt/mt (string-append "@label\n" "@another\n" ":NEXT\n")))
 '(ocm-asm
   (ocm-asm-row (ocm-asm-label #f label #f) (ocm-asm-label #f another #f) (ocm-asm-inst #f NEXT) #f)))
(test-equal?
 "labels work with whitespace"
 (parse-to-datum (__apt/mt (string-append "  @label\n" "      @another\n" " :NEXT\n")))
 '(ocm-asm
   (ocm-asm-row (ocm-asm-label #f label #f) (ocm-asm-label #f another #f) (ocm-asm-inst #f NEXT) #f)))
...
(test-equal?
 ...
 '(ocm-asm (ocm-asm-row (ocm-asm-label #f label) (ocm-asm-label #f another) (ocm-asm-inst #f NEXT) #f)
           (ocm-asm-row (ocm-asm-ref #f refrence) #f)))
...
(test-equal?
 "mixed stuff works"
 (parse-to-datum (__apt/mt (string-append "    (comment)\n"
                                          "    :HI\n"
                                          "    @the-ref"
                                          "    @asdf#14\n"
                                          "    \n"
 ...
 '(ocm-asm #f
           ...
           (ocm-asm-row (ocm-asm-label #f the-ref) (ocm-asm-label #f asdf) (ocm-asm-dta #f 14) #f)
