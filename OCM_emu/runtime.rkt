...
(define-type Instruction-Symbol
             (U 'NOP 'HALT 'SWAP 'NEXT 'GET 'SET 'IFGOTO 'READIN 'SENDOUT 'ADD 'SUBTRACT))
...
(define (num->instruction num)
  (case num
    ...
    [(10) 'SUBTRACT]
    [else
     (raise-argument-error 'ocm-asm
                           "A number 0 through 10 that can be converted into an instruction"
                           num)]))
(module+ test
  (test-exn "Test for invalid integer instruction" exn:fail:contract? (thunk (num->instruction 23)))
  (test-case "Instructions to int and back"
             (for ([inst '(NOP HALT SWAP NEXT GET SET IFGOTO READIN SENDOUT ADD SUBTRACT)])
               (check-eqv? inst (num->instruction (symbol->num (assert inst instruction-symbol?))))))
  (test-case "Instruction to symbol and back"
             (for ([instnum (in-range 0 10)])
               (check-eqv? instnum
                           (symbol->num (num->instruction
                                         (assert instnum exact-nonnegative-integer?)))))))
(define BITTAGE
  :
  (Parameterof Exact-Nonnegative-Integer)
  (make-parameter 6))
(provide BITTAGE)
...
(provide RAM_SIZE)
(define debugger-port
  :
  (Parameterof Output-Port)
...
(define-type OCM-asm-ret
             (List (Vectorof Exact-Nonnegative-Integer)
                   ...
                   Boolean))
(: run-ocm-asm
   (-> #:numbers (Mutable-Vectorof Exact-Nonnegative-Integer)
...
(define (run-ocm-asm #:numbers numbers
  ...
  (define (pad-end-if-needed [valueToCompare bank-#])
    ...
    (unless (or (valueToCompare . < . start-len) (valueToCompare . > . ((RAM_SIZE))))
      (displayln (format "padding...~a" start-len) (debugger-port))
      (set! numbers
            (vector-append numbers
                           (cast (make-vector (- (+ valueToCompare 1) start-len) 0)
  ...
  (displayln (format (string-append "[@] ~a(~a)\t[A] ~a\t[B] ~a\t" "{OVERFLOW}~a")
                     bank-@
                     inst
                     reg-A
                     reg-B
  ...
  (define {GO}
    ...
    (run-ocm-asm #:numbers (cast numbers (Mutable-Vectorof Exact-Nonnegative-Integer))
  ...
  (define {ROTATEONE}
    ...
    (set! bank-# (modulo (+ 1 bank-#) ((RAM_SIZE)))))
  (define {STEP}
  ...
  (define (check-if-reg-A-too-big)
    (when (reg-A . > . ((MAX_INT)))
      (raise-argument-error 'NEXT-or-GET-signal
  ...
  (define {SETA location}
    ...
    (set! reg-A (if (location . < . (vector-length numbers)) (vector-ref numbers location) 0))
  ...
  (case inst
    ...
    [(NEXT)
     ...
     {STEP}]
    [(GET)
     (SETA (cast reg-B Exact-Nonnegative-Integer))
     {STEP}]
    [(SET)
     (pad-end-if-needed (cast reg-B Exact-Nonnegative-Integer))
     (vector-set! numbers reg-B (cast reg-A Exact-Nonnegative-Integer))
     {STEP}]
    [(IFGOTO)
     (if OVERFLOW
         (begin
         ...
         {STEP})]
    [(READIN)
     (set! reg-A (cast (read-byte) Exact-Nonnegative-Integer))
     {STEP}]
    [(SENDOUT)
     (display (integer->char reg-A))
     {STEP}]
    [(ADD)
     (set! reg-A (+ reg-A reg-B))
     (if (> reg-A ((MAX_INT)))
         (begin
           (set! reg-A (modulo reg-A ((MAX_INT))))
           (set! OVERFLOW #t))
         (set! OVERFLOW #f))
     {STEP}]
    [(SUBTRACT)
...
(module+ test
  (require racket/port)
  (: test-pgm (-> (Listof (U Instruction-Symbol Exact-Nonnegative-Integer)) OCM-asm-ret))
  (define (test-pgm program)
    (run-ocm-asm #:numbers (for/vector :
                             (Mutable-Vectorof Exact-Nonnegative-Integer)
                             ([value program])
                             (cond
                               [(instruction-symbol? value) (symbol->num value)]
                               [(exact-nonnegative-integer? value) value]))))
  (test-equal? "Test HALT" (test-pgm '(HALT)) '(#(1) 0 0 0 #f))
  (test-equal? "Test NOP" (test-pgm '(NOP HALT)) '(#(0 1) 1 0 0 #f))
  (test-equal? "Test NEXT" (test-pgm '(NEXT 10 HALT)) '(#(3 10 1) 2 10 0 #f))
  (test-equal? "Test basic SWAP" (test-pgm '(NEXT 10 SWAP HALT)) '(#(3 10 2 1) 3 0 10 #f))
  ...
  (test-case
   "Tests for GET"
   (test-equal? "Test GET" (test-pgm '(NEXT 5 SWAP GET HALT 37)) '(#(3 5 2 4 1 37) 4 37 5 #f))
   (test-equal? "Test GET out of range"
                (test-pgm '(NEXT 8 SWAP NEXT 19 GET HALT))
                '(#(3 8 2 3 19 4 1) 6 0 8 #f)))
  (test-case "Tests for SET"
             (test-equal? "Test SET"
  ...
  (test-case
   "Tests for IFGOTO"
   (test-equal? "Test IFGOTO no jump" (test-pgm '(IFGOTO HALT)) '(#(6 1) 1 0 0 #f))
   (test-equal? "Test IFGOTO"
                ; HALT has id of 1, and is also at 1
                (test-pgm '(NEXT HALT SWAP SUBTRACT IFGOTO NEXT 37 HALT))
                `(#(3 1 2 10 6 3 37 1) 1 ,(- ((MAX_INT)) 1) 1 #t))
   (test-equal?
    "Test IFGOTO overflow"
    (test-pgm '(NEXT 0 SWAP NEXT HALT SET SWAP SUBTRACT NEXT 13 SWAP IFGOTO))
    `(,(vector-append '#(1 0 2 3 1 5 2 10 3 13 2 6) (make-vector (- ((RAM_SIZE)) 12) 0)) 0 1 13 #t)))
  (test-case "Tests for gpio"
             (test-equal? "Test readin"
                          (with-input-from-string (list->string (list (integer->char 35)))
                                                  (thunk (test-pgm '(READIN HALT))))
                          '(#(7 1) 1 35 0 #f))
             (test-equal? "test sendout"
                          (with-output-to-string
                           (thunk (check-equal? (test-pgm '(NEXT 23 SENDOUT HALT))
                                                '(#(3 23 8 1) 3 23 0 #f))))
                          (list->string (list (integer->char 23)))))
  (test-case "Tests for math"
    ...
    (match mode
      ...
      ['binary
       (for/list :
         (Listof String)
         ([number : Exact-Nonnegative-Integer numbers])
         (define binary (~r number #:base 2 #:min-width new-bittage #:pad-string "0"))
         (if big-endian (list->string (reverse (string->list binary))) binary))]
      [else
       (displayln "ERROR! unknown mode")
       '()])
    "\n")))
